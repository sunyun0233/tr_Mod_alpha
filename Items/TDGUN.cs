...
{
	public class TDGUN : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("天顶枪（伪）"); //为你的武器命名
			Tooltip.SetDefault("火力不足恐惧症的良药，次世代的废品");//这是武器的提示语，利用\n进行换行
		}
		public override void SetDefaults()
		{
			//以下是武器物品的基本属性
			Item.damage = 114;//物品的基础伤害
			Item.crit = 36;//物品的暴击率
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            for (int i = -48; i <= 48; i++)
            {
                ...
                Projectile.NewProjectile(source, position, velocity.RotatedBy(0.2f), type == ProjectileID.Bullet ? ModContent.ProjectileType<ModProj.ZHEXIAN>() : type, damage, knockback, player.whoAmI);
                Projectile.NewProjectile(source, position, velocity.RotatedBy(-0.2f), type == ProjectileID.Bullet ? ModContent.ProjectileType<ModProj.ZHEXIAN2>() : type, damage, knockback, player.whoAmI);
                Projectile.NewProjectile(source, position, velocity.RotatedBy(0.8f), type == ProjectileID.Bullet ? ModContent.ProjectileType<ModProj.HUXING2>() : type, damage, knockback, player.whoAmI);
                Projectile.NewProjectile(source, position, velocity.RotatedBy(-0.8f), type == ProjectileID.Bullet ? ModContent.ProjectileType<ModProj.HUXING>() : type, damage, knockback, player.whoAmI);
                Projectile.NewProjectile(source, position, velocity.RotatedBy(0.2f), type == ProjectileID.Bullet ?  ModContent.ProjectileType<ModProj.SIN2>() : type, damage, knockback, player.whoAmI);
                Projectile.NewProjectile(source, position, velocity.RotatedBy(-0.2f), type == ProjectileID.Bullet ? ModContent.ProjectileType<ModProj.SIN>() : type, damage, knockback, player.whoAmI);
                p.hostile = false;
                p.friendly = true;
